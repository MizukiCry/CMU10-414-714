...
void softmax_regression_epoch_cpp(const float *X, const unsigned char *y,
  ...
  for (size_t i = 0; i < m; i += batch) {
    const float *x_d = X + i * n;
    const unsigned char *y_d = y + i;

    float *t = new float[batch * k];
    for (size_t x = 0; x < batch; x++) {
      float sum = 0;
      for (size_t y = 0; y < k; y++) {
        t[x * k + y] = 0;
        for (size_t z = 0; z < n; z++) {
          t[x * k + y] += x_d[x * n + z] * theta[z * k + y];
        }
        t[x * k + y] = std::exp(t[x * k + y]);
        sum += t[x * k + y];
      }
      for (size_t y = 0; y < k; y++) {
        t[x * k + y] /= sum;
    ...
    for (size_t x = 0; x < n; x++) {
      for (size_t y = 0; y < k; y++) {
        float sum = 0;
        for (size_t z = 0; z < batch; z++) {
          sum += x_d[z * n + x] * t[z * k + y];
        }
        theta[x * k + y] -= lr / batch * sum;
      }
    }
  }
  /// END YOUR CODE
